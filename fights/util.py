...
class Counter(dict):
    ...
    def sorted_keys(self):
        ...
        def compare_lambda(x, y):
            return sign(y[1] - x[1])

        compare = compare_lambda
    ...
    def __sub__(self, y):
        """
        Subtracting a counter from another gives a counter with the union of all keys
        and counts of the second subtracted from counts of the first.
...
class TimeoutFunctionExceptionError(Exception):
...
class TimeoutFunction:
    ...
    def handle_timeout(self, signum, frame):
        raise TimeoutFunctionExceptionError()
