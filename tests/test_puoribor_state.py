...
class TestPuoriborState(unittest.TestCase):
    def setUp(self):
        ...
        self.state = self.env.step(self.initial_state, 0, np.array([0, 3, 0]))
        self.state = self.env.step(self.state, 1, np.array([2, 3, 0]))
        self.state = self.env.step(self.state, 0, np.array([0, 3, 1]))
